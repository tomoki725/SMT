# 営業管理システム v4.2.0 - 現在稼働中システム仕様書

## プロジェクト概要
**v4.2.0 新規/既存管理機能追加版 - 2025年8月19日現在**
React.js + Firebase で構築された営業進捗管理システム。完全統合型の営業活動可視化・データ分析プラットフォームとして本格運用中。

## 現在稼働中の主要機能
✅ **認証システム**: SHA256暗号化・30分セッションタイムアウト
✅ **サマリーダッシュボード**: 全体・個人別KPI表示
✅ **案件管理**: 案件・クライアント単位での進捗管理
✅ **アクションログ**: カード形式での活動履歴管理
✅ **ログ記録**: AI分析機能付きアクション入力
✅ **受注管理**: 受注案件の詳細管理
✅ **マスターデータ管理**: 担当者・提案メニューの管理
✅ **KPI管理**: 年度別目標設定・追跡機能
✅ **実績管理**: 予算・実績の月次管理
✅ **週次トピックス**: 個人・全体レベルでの情報共有機能
...
- **案件単位表示**: 商材名、提案メニュー、クライアント、新規/既存、担当者、ステータス、最終接触日（短縮表示）
- **クライアント単位表示**: クライアント名、新規/既存、担当者、メインステータス、最終接触日
- **アクションボタン**: 詳細・編集・追加・削除の4機能
- **フィルター機能**: 検索、ステータス、担当者による絞り込み
- **ステータス色分け**: 8段階のステータスを色で視覚化
- **新規/既存管理**: 案件の新規・既存区分を色分け表示（新規：青、既存：緑、未設定：グレー）
- **編集機能**: 新規/既存の設定・変更が可能
...
const getDealTypeColor = (dealType) => {
  switch (dealType) {
    case '新規': return '#e3f2fd';  // 青背景
    case '既存': return '#e8f5e8';  // 緑背景
    default: return '#f8f9fa';      // グレー背景（未設定）
  }
};
```
...
### 表示項目
**案件単位タブ**:
- 商材名、提案メニュー、クライアント、新規/既存、担当者、ステータス、最終接触日（短縮表示YY/MM/DD）

**クライアント単位タブ**:
- クライアント名、新規/既存、担当者、メインステータス、最終接触日
...
**現在バージョン**: v4.2.0  
**最終更新**: 2025年8月19日  
**開発ステータス**: 本格運用版（新規/既存管理機能付き完全統合営業管理プラットフォーム）  
**本格運用開始**: 2025年8月  
**メンテナンス**: 継続中  
**アクセスURL**: https://psmt-6724f.web.app

## 2025年8月19日現在の稼働状況
- **システム状態**: 正常稼働中
- **認証機能**: 正常動作確認済み
- **全10機能**: 統合運用中
- **データ連携**: Firebase Firestore完全対応
- **UI/UX**: レスポンシブ対応・統一レイアウト適用済み
- **セキュリティ**: SHA256暗号化・セッション管理実装済み
...
## 2025年8月19日 - 新規/既存管理機能追加完了
**実行したタスク**:
1. ✅ ログ記録ページに新規/既存プルダウンを追加
2. ✅ 案件管理の編集モーダルに新規/既存フィールドを追加
3. ✅ 案件管理一覧表示に新規/既存カラムを追加
4. ✅ 新規/既存の色分け表示機能を実装
5. ✅ データベーススキーマにdealTypeフィールドを追加
6. ✅ Firebase Hostingにデプロイして動作確認

**新機能**:
- **新規/既存区分管理**: 全案件に新規・既存の区分を設定・表示可能
- **ログ記録連携**: ログ記録時に新規/既存を任意で選択可能
- **編集機能**: 案件管理の編集画面で新規/既存を変更可能
- **色分け表示**: 新規（青）、既存（緑）、未設定（グレー）で視覚的に区別
- **テーブル表示**: 案件単位・クライアント単位両方で新規/既存情報を表示

**技術実装**:
```javascript
// データベーススキーマ拡張
progressDashboard: {
  dealType: string  // '新規' | '既存' | ''
}
...
const getDealTypeColor = (dealType) => {
  switch (dealType) {
    case '新規': return '#e3f2fd';
    case '既存': return '#e8f5e8';
...
**UI改善**:
- 案件管理テーブルに新規/既存カラムを追加（クライアントと担当者の間）
- 色分けされたバッジ形式で新規/既存を表示
- 編集モーダルでドロップダウン選択により新規/既存を設定可能

**成果**:
- 案件の新規・既存区分を一元管理・視覚化実現
- 営業活動の分析・管理精度向上
- ユーザビリティ向上による運用効率化
- https://psmt-6724f.web.app への正常デプロイ確認
